...
The development server prefers port 3000 and may move to the next available
port. Check the terminal output for the actual URL. You can force a port:

```powershell
$env:PORT="3010"; pnpm dev
```
...
If ports `3307`, `1025`, or `8025` are already in use, stop the conflicting
service or adjust `docker-compose.e2e.yml` and `.env.e2e` together.

## Email Links Point To The Wrong Host

Set `APP_URL` to the externally reachable URL of the instance. For local E2E,
`APP_URL` and `BASE_URL` should usually both be `http://localhost:3001`.
